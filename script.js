document.addEventListener('DOMContentLoaded', () => {    
    //* Referencia dos elementos HTML necessários *//   
    ...
    //* Função que gera e exibe as citações *//
    function displayRandomQuote() {
        const randomIndex = Math.floor(Math.random() * quotes.length);
        const randomQuote = quotes[randomIndex];

        numberQuote.textContent = randomIndex + 1;
        quoteText.querySelector('p').textContent = randomQuote.citacao;
        authorText.textContent = "- " + randomQuote.autor;

        console.log("Citação atualizada!");
    }
